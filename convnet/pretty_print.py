...
def filter_expr(expr, threshold=0.01):
    ...
    for arg in expr.args:
        if arg.is_constant(simplify=False) and abs(arg) > threshold:   # hack way to check if it's a number
            expr_new = expr_new + arg
        elif not arg.is_constant(simplify=False) and abs(arg.args[0]) > threshold:
            expr_new = expr_new + arg
    return expr_new
